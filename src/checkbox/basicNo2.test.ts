...
describe('test formio stats', () => {
    it('properly gives checkbox statistics for multiple attributes in and out of the order they occur', () => {
       ...
       const res = distribution([formioResult1, formioResult2, formioResult3, formioResult4], ["under18", "male", "female", "haveEpilepsy","married","single"])
       expect.arrayContaining(res.trueArr)
       expect([50, 100, 0, 75, 75, 25]).toEqual(expect.arrayContaining(res.trueArr));
       expect.arrayContaining(res.falseArr)
       expect([50, 0, 100, 25, 25, 75]).toEqual(expect.arrayContaining(res.falseArr));
    })
})